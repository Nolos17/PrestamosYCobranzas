        ...
        <div class="card card-outline card-primary">
            <div class="card-header">
                ...
                <div class="card-tools">
                    <a href="{{ url('/admin/cuotas/create') }}" class="btn btn-primary">Crear Nuevo</a>
                </div>
            </div>
            <div class="card-body">
                <table id=example1 class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $pagos->cliente->nro_documento }}</td>
                            <td>{{ $pagos->cliente->apellidos . ' ' . $pagos->cliente->nombres }}</td>
                            <td>{{ $pagos->total_pago }}</td>
                            <td>{{ $pagos->fecha_pago }}</td>
                            <td>{{ $pagos->metodo_pago }}</td>
                            <td>
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    <!-- Botón de Ver -->
                                    <a href="{{ url('/admin/cuotas/' . $pagos->id) }}" class="btn btn-info btn-sm"
                                        style="border-radius: 4px 0px 0px 4px">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <!-- Botón de imprimir contrato -->
                                    <a href="{{ url('/admin/cuotas/recibos/' . $pagos->id) }}"
                                    ...
                                    <!-- Botón de Editar -->
                                    <a href="{{ url('/admin/cuotas/' . $pagos->id . '/edit') }}"
                                    ...
                                    <form action="{{ url('/admin/cuotas/' . $pagos->id) }}" method="post"
                                        id="miFormulario{{ $pagos->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-danger btn-sm"
                                            style="border-radius: 0px 4px 4px 0px "
                                            onclick='preguntar("{{ $pagos->id }}");'>
