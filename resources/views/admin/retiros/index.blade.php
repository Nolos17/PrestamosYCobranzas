        ...
        <div class="col-md-12">
            <div class="card card-outline card-primary">
                <div class="card-header">
                    <h3 class="card-title">Retiros Registrados</h3>
                    <div class="card-tools">
                        <a href="{{ url('/admin/retiros/create') }}" class="btn btn-primary">Crear Nuevo</a>
                    </div>
                </div>
                <div class="card-body">
                    <table id=example1 class="table">
                        ...
                        <tbody>
                            @php
                                $contador = 1;
                            @endphp
                            @foreach ($retiro as $retiros)
                                <tr>
                                    <td style="text-align: center">{{ $contador++ }}</td>
                                    <td>{{ $retiros->cliente->nro_documento }}</td>
                                    <td>{{ $retiros->cliente->apellidos . ' ' . $retiros->cliente->nombres }}</td>
                                    <td>{{ $retiros->total_retiro }}</td>
                                    <td>{{ $retiros->fecha_retiro }}</td>
                                    <td>{{ $retiros->metodo_retiro }}</td>
                                    <td>
                                        <div class="btn-group" role="group" aria-label="Basic example">
                                            <!-- Botón de Ver -->
                                            <a href="{{ url('/admin/retiros/' . $retiros->id) }}"
                                                class="btn btn-info btn-sm" style="border-radius: 4px 0px 0px 4px">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <!-- Botón de imprimir contrato -->
                                            <a href="{{ url('/admin/retiros/recibos1/' . $retiros->id) }}"
                                                class="btn btn-warning btn-sm" style="border-radius: 0px 0px 0px 0px">
                                                <i class="fas fa-print"></i>
                                            </a>

                                            <!-- Botón de Editar -->
                                            <a href="{{ url('/admin/retiros/' . $retiros->id . '/edit') }}"
                                            ...
                                            <form action="{{ url('/admin/retiros/' . $retiros->id) }}" method="post"
                                                id="miFormulario{{ $retiros->id }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" class="btn btn-danger btn-sm"
                                                    style="border-radius: 0px 4px 4px 0px " hidden
                                                    onclick='preguntar("{{ $retiros->id }}");'>
